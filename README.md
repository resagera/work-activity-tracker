...
По умолчанию `Makefile` использует Go toolchain:
...
```bash
make build GO_BIN=/path/to/go
```

### Linux systemd user service

Для автозапуска трекера и tray в Linux:

```bash
make install-linux-user-services
```
...
* устанавливает `systemd --user` unit'ы из `deploy/systemd/user`;
...
Обновить бинарники и перезапустить сервисы:

```bash
make update-linux-user-services
```

Полезные команды:

```bash
make status-linux-user-services
make logs-linux-user-services
make restart-linux-user-services
make stop-linux-user-services
```

Старый скрипт оставлен как обёртка:
...
Заглушки для будущей настройки автозапуска:

```bash
make install-autostart-macos
