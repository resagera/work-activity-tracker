...
```bash
curl -X POST "http://127.0.0.1:8080/activity-types/add?name=проектирование"
```
...
```bash
curl -X POST "http://127.0.0.1:8080/inactivity-types/add?name=перекус"
```
...
Также в Web UI можно выбрать текущий тип активности и неактивности, а также добавить свои типы.
